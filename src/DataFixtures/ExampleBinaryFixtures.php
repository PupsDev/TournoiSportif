...
class ExampleBinaryFixtures extends Fixture
{
    protected $encoder;
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $organizer->setEmail("lucas.durabot@orange.fr");
        $organizer->setPassword( $this->encoder->getEncoder($organizer)->encodePassword("password", ""));
        $organizer->setFirstName("Lucas");
        $organizer->setSurname("Durabot");
        $organizer->setStructure("Adversity");
        $organizer->setRoles(array('ROLE_ORGANIZER'));

    	$date = \DateTime::createFromFormat('Y-m-d H:i:s', "2020-12-05 09:30:00");
    	$event = new Event();
    	$event->setName("ALGS Apex");
    	$event->setSport("sports_esports");
    	$event->setDate($date);
    	$event->setLocation("Bordeaux");
    	$event->setPlayerPerTeam(3);
        $event->setOrganizer($organizer);
        $event->setTeamPerTurnament(8);
        $organizer->addEvent($event);

    	$turnament = new Turnament();
    	$turnament->setName("Tournoi Binaire 8 équipes");

    	$levels = [5,1,4,3,2,5,1,5];
