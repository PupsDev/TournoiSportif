...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $organizer->setEmail("michel.meynard@umontpellier.fr");
        $organizer->setPassword( $this->encoder->getEncoder($organizer)->encodePassword("password", ""));
        $organizer->setFirstName("Michel");
        $organizer->setSurname("Meynard");
        $organizer->setStructure("UM");
        $organizer->setRoles(array('ROLE_ORGANIZER'));

    	$date = \DateTime::createFromFormat('Y-m-d H:i:s', "2020-07-14 14:00:00");
    	$event = new Event();
    	$event->setName("FêteNat");
    	$event->setSport("sports_volleyball");
    	$event->setDate($date);
    	...
    	$turnament->setName("Principal");
        //         a b c d e f g
    	$levels = [5,1,4,3,2,5,1];

    	//7 équipes dans le tournois
    	for($i = 0 ; $i <7; $i++)
    	{
    		$team = new Team();
            // a -> g
